...
export function renderDocumentsView(state) {
  return `<h2>Documents</h2>
    ...
    ${card('Current Stage Detail', '<p class="subtle">Next up: reviewer package assembly, citation alignment, and approval routing preview.</p><div class="row mt-12"><button class="ghost" disabled title="Not implemented yet">Open reviewer packet</button><button class="ghost" disabled title="Not implemented yet">Preview extracted graph</button></div>')}
...
export function renderFeedbackView(state) {
  return `<h2>Feedback</h2>
    <div class="grid cols-2">
      ${card('Review Surface', `<div class="list">${state.feedbackState.reviewItems.map((item) => `<div class="row"><div><strong>${item.item}</strong><p class="subtle">Owner ${item.owner}</p></div><span class="status ${item.status === 'applied' ? 's-healthy' : 's-warn'}">${item.status}</span></div>`).join('')}</div><div class="row mt-12"><button class="solid" data-action="apply-feedback">Apply pending feedback</button><button class="ghost" disabled title="Not implemented yet">Open review diff</button></div>`) }
...
export function renderArtifactsView(state) {
  return `<h2>Artifacts</h2>
    ...
    <div class="grid cols-2 mt-12">
      ...
      ${card('Meeting-ready Outputs', `<div class="list">${state.artifactState.meetingOutputs.map((m) => `<div>${m}</div>`).join('')}</div><div class="row mt-12"><button class="solid" disabled title="Not implemented yet">Export selected outputs</button></div>`) }
    </div>`;
}
