...
export function renderFoundryView(state) {
  ...
  <div class="grid cols-2 mt-12">
    ...
    ${card('Foundry Plan', `<div class="row"><span class="subtle">${state.foundryState.planApproved ? 'Plan approved and ready to run.' : 'Approve and run to continue processing.'}</span><span class="status ${state.foundryState.planApproved ? 's-healthy' : 's-warn'}">${state.foundryState.planApproved ? 'approved' : 'pending'}</span></div><div class="list mt-12">${state.foundryState.plan.map((p) => `<div class="row"><span>${p.title}</span><span class="subtle">${p.enabled ? 'enabled' : 'disabled'}</span></div>`).join('')}</div><div class="row mt-12"><button class="ghost" data-action="approve-plan">Approve all</button><button class="ghost" data-action="toggle-optional">Toggle optional</button><button class="solid" data-action="run-plan">Run plan</button></div>`) }
  </div>
