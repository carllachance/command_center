...
export function renderLensesView(state) {
  ...
  return `<h2>Lenses</h2>
    ...
    <div class="grid cols-2 mt-12">
      ${card('Lens Hero', `<div class="hero-panel"><p><strong>${hero.title}</strong></p><p>${hero.summary}</p><p class="subtle">${hero.caveat}</p><p class="subtle">Last run: ${hero.lastRun}</p><div class="row wrap">${hero.actions.map((action) => `<button class="ghost" ${action === 'Send to Ad Hoc' ? 'data-action="send-hero-to-adhoc"' : 'disabled title="Not implemented yet"'}>${action}</button>`).join('')}</div></div>`)}
